...
to setup
  ...
  [
    ...
    set templist []
    set model []
    ; random postavljanje liste za  kupovinu
    list-setup
    setxy -8 -6
    set heading 90
    show shopping-list
  ...
  ask n-of ((random 5) + 3) patches [ set pcolor orange ]
  ask n-of ((random 5) + 3) patches [ set pcolor red ]
  ask n-of ((random 5) + 3) patches [ set pcolor green ]
  ask n-of ((random 5) + 3) patches [ set pcolor blue ]
  ask n-of ((random 5) + 3) patches [ set pcolor yellow ]
  ask n-of ((random 5) + 3) patches [ set pcolor grey ]
  ask n-of ((random 5) + 3) patches [ set pcolor white ]
...
; NLZ03 funkcija kretanja prema najblizem artiklu za kupovinu
to walk
  if empty? color-list ; staje ako nema vise artikala za traziti
  [
    stop
  ]
  ...
  foreach model
  [
    ; uzimau obzir samo one koje jos nije pokupio
    xy ->  if (distance(patch item 0 xy item 1 xy) < minDist) and (member? [pcolor] of patch item 0 xy item 1 xy color-list)
    [
...
; kreiranje slučajne liste za kupovinu
; FUNKCIJA ZA ZADAĆU
to list-setup
  repeat (random 6) + 1 [

    set rnd random 7
     if (rnd = 0) and not (member? "Orange" shopping-list)
    [
      set shopping-list lput "Orange" shopping-list
      set color-list lput orange color-list
    ]
    if (rnd = 1) and not (member? "Red" shopping-list)
    [
      set shopping-list lput "Red" shopping-list
      set color-list lput red color-list
    ]
    if (rnd = 2) and not (member? "Green" shopping-list)
    [
      set shopping-list lput "Green" shopping-list
      set color-list lput green color-list
    ]
    if (rnd = 3) and not (member? "Blue" shopping-list)
    [
      set shopping-list lput "Blue" shopping-list
      set color-list lput blue color-list
    ]
    if (rnd = 4) and not (member? "Grey" shopping-list)
    [
      set shopping-list lput "Grey" shopping-list
      set color-list lput grey color-list
    ]
    if (rnd = 5) and not (member? "Yellow" shopping-list)
    [
      set shopping-list lput "Yellow" shopping-list
      set color-list lput yellow color-list
    ]
    if (rnd = 6) and not (member? "White" shopping-list)
    [
      set shopping-list lput "White" shopping-list
      set color-list lput white color-list
    ]
...
to make-model
  ...
  [
    set templist []
    if (member? [pcolor] of patch x-pos y-pos color-list)
    [
...
to shop
  ...
  [
    ...
    [
      ...
      set model remove-item index model ; izbacuje kordinatu iz modela NLZ03
      set color-list remove orange color-list ; izbacuje boju iz traženih boja NLZ03
      show shopping-cart
    ]
  ]
